...
class MainController:
    ...
    def initialize_system(self) -> bool:
        ...
        try:
            ...
            # 初始化报告生成器
            self.report_generator = ReportGenerator(omit_empty_categories=True)
            ...
            if telegram_command_config.enabled and auth_config.TELEGRAM_BOT_TOKEN:
                try:
                    from .reporters.telegram_command_handler import (
    ...
    def get_semantic_search_service(self) -> Optional[Any]:
        if self.semantic_search_service is not None:
            return self.semantic_search_service

        if self.config_manager is None or self.content_repository is None:
            self.logger.info("语义搜索服务依赖未就绪，跳过初始化")
            return None

        if self.embedding_service is None:
            self.logger.info("EmbeddingService未初始化，语义搜索服务不可用")
            return None

        semantic_search_config = self.config_manager.get_semantic_search_config()
        if not semantic_search_config.enabled:
            self.logger.info("语义搜索未启用，跳过SemanticSearchService初始化")
            return None
        ...
        self.semantic_search_service = SemanticSearchService.from_llm_config_payload(
        ...
        try:
            ...
            try:
                ...
                if execution_interval <= 0 or time_window_hours <= 0:
                    validation_result["errors"].append(
                        "execution_interval和time_window_hours必须为正整数"
                    )
                    validation_result["valid"] = False
            except Exception as e:
                validation_result["errors"].append(
                    f"无法获取execution_interval或time_window_hours: {e}"
                )
                validation_result["valid"] = False
            ...
            if not skip_analysis_auth_validation:
                ...
                try:
                    ...
                    self._validate_runtime_auth(
                        auth_config,
                        llm_config,
                        mode=normalized_scope or "analysis-service",
                    )
                except Exception as e:
            ...
            self.logger.info(
                f"前提条件验证完成: {'通过' if validation_result['valid'] else '失败'}"
            )
    ...
    def _resolve_provider_credentials(
        ...
        for provider_name in self._required_llm_providers(llm_config):
            ...
            credentials[provider_record.name] = getattr(
                auth_config, provider_record.env_var, ""
            ).strip()
        return credentials
    ...
    def run_once(
        ...
        trigger_chat_id: Optional[str] = None,
    ) -> ExecutionResult:
        """
        执行一次完整的工作流
        ...
        try:
            ...
            result = self.coordinate_workflow(
                trigger_chat_id=trigger_chat_id, is_manual=(trigger_type == "manual")
            )
            ...
            # 记录执行日志
            self.log_execution_cycle(
                start_time, end_time, "success" if result["success"] else "failed"
            )

            self.logger.info(
                f"工作流执行完成 {execution_id}: {'成功' if result['success'] else '失败'}"
            )
    ...
    def coordinate_workflow(
        self, trigger_chat_id: Optional[str] = None, is_manual: bool = False
    ) -> Dict[str, Any]:
        """
        协调完整的工作流程
        ...
        try:
            ...
            report_result = self._execute_reporting_stage(
                categorized_items, analysis_results, crawl_status, time_window_hours
            )
    ...
    def _execute_analysis_stage(
        ...
        try:
            ...
            self.logger.info(
                f"从数据库获取到时间窗口内的 {len(all_content_items)} 个内容项进行分析"
            )
            self.logger.info(
                f"其中本次新爬取 {len(newly_crawled_items)} 个，历史数据 {len(all_content_items) - len(newly_crawled_items)} 个"
            )
    ...
    def _execute_reporting_stage(
        ...
        try:
            ...
            report_content = self.report_generator.generate_telegram_report(
                analyzed_data, crawl_status
            )

            result.update({"success": True, "report_content": report_content})
    ...
    def _execute_sending_stage(
        ...
        try:
            ...
            if target_chat_id:
                ...
                send_result = self.telegram_sender.send_report_to_chat(
                    report_content, target_chat_id
                )
            else:
                # 定时任务报告，发送到配置的频道
                self.logger.info(
    ...
    def _scheduler_loop(self, interval_seconds: int) -> None:
        ...
        if not self._last_scheduled_time and self.execution_history:
            ...
            for result in reversed(self.execution_history):
                if result.trigger_type == "scheduled":
                    ...
                    self.logger.info(
                        f"从执行历史恢复上次调度时间: {self._last_scheduled_time.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
                    break
        ...
        if not self._last_scheduled_time:
            ...
            self.logger.info(
                f"初始化上次调度时间为当前时间: {self._last_scheduled_time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if wait_seconds <= 0:
                    self.logger.info(
                        f"下次执行时间 {next_execution.strftime('%Y-%m-%d %H:%M:%S')} 已过，立即执行"
                    )
                    wait_seconds = 0
                else:
                    self.logger.info(
                        f"下次执行时间: {next_execution.strftime('%Y-%m-%d %H:%M:%S')}，等待 {wait_seconds:.0f} 秒"
                    )
                ...
                if result.success:
                    ...
                    self.logger.error(
                        f"定时执行失败 (连续失败: {consecutive_failures}/{max_consecutive_failures}): {'; '.join(result.errors)}"
                    )
                ...
                consecutive_failures += 1
                self.logger.error(
                    f"调度器循环异常 (连续失败: {consecutive_failures}/{max_consecutive_failures}): {str(e)}"
                )
                self.logger.debug(traceback.format_exc())
                ...
                if consecutive_failures >= max_consecutive_failures:
                    ...
                    self.logger.warning(
                        f"连续异常{consecutive_failures}次，等待{backoff_time}秒后继续"
                    )
                    if self._stop_event.wait(backoff_time):
                        break
                else:
    ...
    def analyze_by_time_window(
        ...
        try:
            ...
            content_items = self.content_repository.get_content_items_since(
                since_time=since_time, max_hours=time_window_hours
            )
            self.logger.info(
                f"从数据库获取到 {len(content_items)} 个内容项进行时间窗口分析 (execution_id: {execution_id})"
            )
    ...
    def get_markdown_report_for_api(
        self, time_window_hours: int, user_id: Optional[str] = None
    ) -> str:
        """
        获取API用的Markdown报告
