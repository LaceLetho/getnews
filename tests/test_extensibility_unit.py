...
from crypto_news_analyzer.analyzers.prompt_manager import (
    PromptManager,
    ...
    def validate_config(self, config: Dict[str, Any]) -> bool:
        ...
        if missing:
            raise ConfigValidationError(
                f"缺少字段: {missing}", "custom", config.get("name", "")
            )
        return True
...
class TestNewDataSourceRegistration:
    ...
    def test_register_custom_data_source(self):
        ...
        source = self.factory.create_source(
            "custom", time_window_hours=24, custom_param="test_value"
        )
...
class TestDynamicConfigurationUpdates:
    ...
    def test_missing_configuration_file_handling(self):
        """测试缺失配置文件处理"""
        missing_config_path = os.path.join(self.temp_dir, "missing_config.jsonc")
...
class TestExtensibilityIntegration:
    ...
    def setup_method(self):
        """测试前设置"""
        self.temp_dir = tempfile.mkdtemp()
        self.config_path = os.path.join(self.temp_dir, "integration_config.jsonc")
        self.factory = DataSourceFactory()
    ...
    def test_new_data_source_with_config_integration(self):
        ...
        source = self.factory.create_source(
            "custom", time_window_hours=config["time_window_hours"]
        )
