...
class TestTelegramSendingReliabilityProperties:
    ...
    def test_telegram_sending_with_invalid_config(self, config: TelegramConfig, report: str):
        """
        属性测试：无效配置时的处理
        
        验证当Telegram配置无效时，系统能够正确拒绝发送
        """
        async def run_test():
            sender = TelegramSender(config)
            ...
            async with sender:
                # 模拟配置验证失败
                with patch.object(sender, 'validate_configuration', 
                                return_value=SendResult(success=False, error_message="配置无效")):
                    result = await sender.send_report(report)
            
            # 验证：配置无效时应该拒绝发送
            assert not result.success, "配置无效时发送应该失败"
            assert "配置验证失败" in result.error_message, "应该包含配置验证失败的错误信息"
            assert result.message_id is None, "配置无效时不应该有消息ID"
            assert result.parts_sent == 0, "配置无效时不应该发送任何部分"
    ...
    @settings(max_examples=10, deadline=None)
    def test_retry_mechanism_reliability(self, config: TelegramConfig, report: str):
        """
        属性测试：重试机制的可靠性
    ...
    @given(
        config=valid_telegram_config(),
        markdown_text=st.text(min_size=10, max_size=1000)
    )
    @settings(max_examples=15, deadline=None)
    def test_markdown_formatting_consistency(self, config: TelegramConfig, markdown_text: str):
        """
        属性测试：Markdown格式化的一致性
        
        验证Markdown格式化不会破坏文本结构
        """
        sender = TelegramSender(config)
        
        # 格式化文本
        formatted_text = sender.format_for_telegram(markdown_text)
        
        # 验证：格式化后的文本不为空（除非原文本为空）
        if markdown_text.strip():
            assert formatted_text.strip(), "格式化后的文本不应为空"
        
        # 验证：格式化后的文本长度合理
        assert len(formatted_text) >= 0, "格式化后的文本长度应该非负"
        
        # 验证：格式化是幂等的（多次格式化结果相同）
        double_formatted = sender.format_for_telegram(formatted_text)
        # 注意：由于格式化可能包含转义，这里只验证长度不会无限增长
        assert len(double_formatted) <= len(formatted_text) * 2, "重复格式化不应导致长度无限增长"
    
    @given(config=valid_telegram_config())
    @settings(max_examples=10, deadline=None)
    def test_configuration_validation_reliability(self, config: TelegramConfig):
        """
        属性测试：配置验证的可靠性
        
        验证配置验证能够正确识别有效和无效的配置
        """
        async def run_test():
            # 模拟Bot Token验证成功
            mock_bot_response = AsyncMock()
            ...
            with patch('aiohttp.ClientSession.post') as mock_post:
                # 设置mock返回不同的响应
                mock_post.return_value.__aenter__.side_effect = [mock_bot_response, mock_channel_response]
                
                sender = TelegramSender(config)
                
                async with sender:
                    result = await sender.validate_configuration()
                
                # 验证：有效配置应该通过验证
                assert result.success, f"有效配置验证应该成功: {result.error_message}"
                assert result.error_message is None, "成功验证不应该有错误信息"
    ...
    @settings(max_examples=10, deadline=None)
    def test_backup_creation_reliability(self, config: TelegramConfig, report: str):
        """
        属性测试：备份创建的可靠性
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 临时修改备份目录
            original_backup_method = sender.save_report_backup
            ...
            try:
                # 创建备份
                backup_path = sender.save_report_backup(report, backup_filename)
                ...
                # 标准化行尾符进行比较
                normalized_backup = backup_content.replace('\r\n', '\n').replace('\r', '\n')
                normalized_report = report.replace('\r\n', '\n').replace('\r', '\n')
                
                assert normalized_backup == normalized_report, "备份内容应该与原始报告一致"
                
            finally:
                # 恢复原始方法
                sender.save_report_backup = original_backup_method
