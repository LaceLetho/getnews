...
class _StubContentRepository:
    ...
    def semantic_search_by_similarity(
        self, query_embedding, since_time, max_hours, limit
    ):
        self.calls.append(
            {
...
def test_query_planner_caps_unique_subqueries_and_keeps_original_query(monkeypatch):
    ...
    monkeypatch.setattr(
        service, "_llm_complete", lambda *_args, **_kwargs: next(responses)
    )
...
def test_global_retained_set_is_capped_to_200_unique_items(monkeypatch):
    first_batch = [
        (_build_item(f"item-{index}", minutes=index), 0.9) for index in range(180)
    ]
    second_batch = [
        (_build_item(f"item-{index}", minutes=500 - index), 0.8)
...
def test_no_match_returns_compact_non_error_report_shape(monkeypatch):
    service = _build_service(repository=_StubContentRepository([[]]))
    responses = iter(
        ['{"normalized_intent":"SOL生态空投","subqueries":["sol airdrop"]}']
    )
    monkeypatch.setattr(
        service, "_llm_complete", lambda *_args, **_kwargs: next(responses)
    )
...
def test_report_builder_uses_fixed_sections():
