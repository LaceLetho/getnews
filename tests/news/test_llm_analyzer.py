...
class TestLLMAnalyzer:
    ...
    def test_initialization_with_api_keys(self):
        ...
        analyzer = LLMAnalyzer(
            provider_credentials={"kimi": "test_kimi_key", "grok": "test_grok_key"}, mock_mode=False
        )
    ...
    def test_deepseek_thinking_level_high_maps_to_enabled(self):
        runtime = resolve_model_runtime(
            ModelConfig(
        ...
        analyzer = LLMAnalyzer(model=runtime, mock_mode=True)

        assert analyzer._build_model_extra_body(runtime) == {"thinking": {"type": "enabled"}}

    def test_deepseek_thinking_level_disabled_stays_disabled(self):
        runtime = resolve_model_runtime(
            ModelConfig(
                provider="opencode-go",
                name="deepseek-v4-pro",
        ...
        assert analyzer._build_model_extra_body(runtime) == {"thinking": {"type": "disabled"}}
    ...
    def test_build_user_prompt_with_context(self, mock_content_items, mock_market_snapshot):
        ...
        user_prompt = analyzer._build_user_prompt_with_context(
            mock_content_items, mock_market_snapshot
        )
    ...
    def test_update_config(self):
        ...
        # 更新配置
        analyzer.update_config(temperature=0.5, batch_size=20, max_tokens=2000)
    ...
    def test_classify_content_dynamic_mock_mode(self):
        ...
        result = analyzer.classify_content_dynamic(
            content="比特币价格突破新高", market_context="市场处于上涨趋势"
        )
    ...
    def test_kimi_content_filter_fallback_uses_configured_grok_with_search(
        ...
        fallback_result = BatchAnalysisResult(
            results=[
        ...
        fallback_call = analyzer.structured_output_manager.force_structured_response.call_args_list[
            1
        ]
