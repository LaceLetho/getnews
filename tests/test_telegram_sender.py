...
class TestTelegramSender:
    ...
    @pytest.mark.asyncio
    async def test_send_message_part_retries_without_parse_mode_on_entity_error(self):
        calls = []
        ...
        async with self.sender:
            result = await self.sender._send_message_part(
                "# 标题\n未闭合 *markdown", 1, 1
            )
    ...
    async def test_retry_warning_redacts_telegram_token(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger=self.sender.logger.name):
            async with self.sender:
                result = await self.sender._send_message_part(
                    "测试消息", 1, 1, max_retries=1
                )

        assert result.success is False
        assert token not in caplog.text
        assert (
            "https://api.telegram.org/bot[REDACTED]/sendMessage failed" in caplog.text
        )
    ...
    async def test_send_report_redacts_telegram_token_from_exception(self, caplog):
        ...
        self.sender.validate_configuration = AsyncMock(
        ...
        assert "https://api.telegram.org/bot[REDACTED]/sendMessage" in (
...
class TestSendResult:
    ...
    def test_send_result_failure(self):
        """测试失败结果"""
        result = SendResult(
            success=False, error_message="发送失败", parts_sent=0, total_parts=1
        )
