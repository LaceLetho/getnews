...
def valid_rest_api_source(draw):
    ...
    return {
        ...
        "response_mapping": {
            "title_field": draw(SAFE_FIELD_NAME),
            "content_field": draw(SAFE_FIELD_NAME),
            "url_field": draw(SAFE_FIELD_NAME),
            "time_field": draw(SAFE_FIELD_NAME),
        },
    }
...
class TestConfigFileManagementProperties:
    ...
    @given(
        ...
        modification_type=st.sampled_from(["llm", "sources", "storage"]),
    )
    @settings(max_examples=50, deadline=None)
    def test_config_modification_support(
        self, original_config: Dict[str, Any], modification_type: str
    ):
        """
        **属性 3: 配置文件管理 - 配置修改支持**
        **验证: 需求 2.5**
        ...
        # 根据修改类型进行不同的修改
        if modification_type == "llm":
            modified_config["llm_config"] = modified_config["llm_config"].copy()
            modified_config["llm_config"]["batch_size"] = (
                original_config["llm_config"]["batch_size"] + 1
            )
            ...
            modified_config["storage"]["retention_days"] = (
                original_config["storage"]["retention_days"] + 1
            )
    ...
    def test_url_format_validation(self, config_data: Dict[str, Any]):
        ...
        **验证: 需求 2.12 (相关需求)**
        ...
        for source in config_data.get("rest_api_sources", []):
            ...
            if (
                not (endpoint.startswith("http://") or endpoint.startswith("https://"))
                or len(endpoint) < 10
            ):
                has_invalid_url = True
                break
