"""
Unit tests for task 8.2: Update _handle_status_command() method

Tests verify that _handle_status_command():
...
@pytest.mark.asyncio
async def test_handle_status_command_extracts_chat_context(handler):
    """Test that _handle_status_command calls _extract_chat_context at the start"""
        ...
        with patch.object(handler, 'is_authorized_user', return_value=True):
            with patch.object(handler, 'handle_status_command', return_value="✅ 状态正常"):
                with patch.object(handler, '_log_authorization_attempt'):
                    await handler._handle_status_command(update, context)
...
@pytest.mark.asyncio
async def test_handle_status_command_uses_context_fields(handler):
    """Test that _handle_status_command extracts and uses fields from ChatContext"""
...
@pytest.mark.asyncio
async def test_handle_status_command_logs_with_chat_context(handler):
...
@pytest.mark.asyncio
async def test_handle_status_command_calls_log_authorization_attempt_on_failure(handler):
...
@pytest.mark.asyncio
async def test_handle_status_command_calls_log_authorization_attempt_on_success(handler):
...
@pytest.mark.asyncio
async def test_handle_status_command_no_validate_user_permissions_call(handler):
...
@pytest.mark.asyncio
async def test_handle_status_command_consistent_error_message(handler):
...
@pytest.mark.asyncio
async def test_handle_status_command_handles_context_extraction_error(handler):
...
@pytest.mark.asyncio
async def test_handle_status_command_logs_error_with_context(handler):
    """Test that errors are logged with chat context information"""
    update = create_mock_update("123456789", "testuser", "group", "-100123456789")
    context = Mock(spec=ContextTypes.DEFAULT_TYPE)
    
    with patch.object(handler, 'is_authorized_user', return_value=True):
        with patch.object(handler, 'handle_status_command') as mock_handle:
...
@pytest.mark.asyncio
async def test_handle_status_command_private_chat_flow(handler):
...
@pytest.mark.asyncio
async def test_handle_status_command_group_chat_flow(handler):
...
@pytest.mark.asyncio
async def test_handle_status_command_supergroup_chat_flow(handler):
    """Test complete flow for supergroup chat"""
    update = create_mock_update("123456789", "testuser", "supergroup", "-100987654321")
    context = Mock(spec=ContextTypes.DEFAULT_TYPE)
    ...
    with patch.object(handler, 'is_authorized_user', return_value=True):
        ...
        # Verify authorization was logged with supergroup chat context
        mock_log_auth.assert_called_once()
        call_kwargs = mock_log_auth.call_args[1]
        assert call_kwargs['chat_type'] == 'supergroup'
        assert call_kwargs['chat_id'] == '-100987654321'
        assert call_kwargs['authorized'] == True


@pytest.mark.asyncio
async def test_handle_status_command_unauthorized_in_group(handler):
    """Test that unauthorized users are rejected in group chats"""
    update = create_mock_update("999999999", "unauthorized", "group", "-100123456789")
    context = Mock(spec=ContextTypes.DEFAULT_TYPE)
    ...
    with patch.object(handler, 'is_authorized_user', return_value=False):
        ...
        # Verify authorization was logged with authorized=False
        mock_log_auth.assert_called_once()
        call_kwargs = mock_log_auth.call_args[1]
        assert call_kwargs['authorized'] == False
        assert call_kwargs['reason'] == "user not in authorized list"
