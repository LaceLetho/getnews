...
- [x] 8. 实现Telegram适配的报告生成和发送系统
  ...
  - [x] 8.2 重构报告生成器支持动态分类
    ...
    - _需求: 7.2, 7.4, 7.5, 7.7, 7.11, 7.14_
...
- [ ] 10. 实现Docker化部署和内部定时调度
  ...
  - [x] 10.3 创建Docker入口脚本和容器配置
    - 编写docker-entrypoint.sh支持容器启动和管理
    - 配置数据卷挂载（配置、日志、数据）
    - 确保bird工具在容器中正确安装和配置
    - 支持一次性执行模式和持续运行的定时调度模式
    - 实现容器健康检查机制
    - _需求: 9.5, 9.9, 9.10_
  
  - [x] 10.4 实现容器信号处理和状态管理
    ...
    - _需求: 9.11, 9.12, 9.13, 9.15, 9.16, 9.17, 9.18_
...
- [x] 11. 实现已发送消息缓存和去重功能
  - [x] 11.1 创建缓存管理器
    - 实现SentMessageCacheManager类，管理已发送消息缓存
    - 使用SQLite存储缓存数据（summary、category、time、sent_at）
    - 实现cache_sent_messages方法保存已发送消息
    - 实现get_cached_messages方法读取24小时内的缓存
    - 实现cleanup_expired_cache方法清理过期记录
    - _需求: 17.1, 17.2, 17.3, 17.4, 17.12_
  
  - [x] 11.2 实现缓存格式化和提示词集成
    - 实现format_cached_messages_for_prompt方法
    - 格式化缓存消息为简洁文本：`- [时间] [分类] 摘要`
    - 在LLMAnalyzer中集成缓存管理器
    - 在build_system_prompt中替换${outdated_news}占位符
    - 缓存为空时填充"无"
    - _需求: 17.5, 17.6, 17.7, 17.8, 17.10_
  
  - [ ] 11.3 集成缓存到执行流程
    - 在ExecutionCoordinator中集成缓存管理器
    - 报告发送成功后调用cache_sent_messages
    - 系统启动时调用cleanup_expired_cache
    - 手动触发的/run命令也使用缓存机制
    - 实现缓存统计和监控
    - _需求: 17.9, 17.13, 17.14_
...
- [x] 13. 创建部署配置和示例文件
  - [x] 13.1 创建部署配置和示例文件
    ...
    - 提供Docker运行示例和配置模板（一次性执行、持续调度和命令监听三种模式）
    - _需求: 9.4, 9.5, 9.8, 9.14, 16.10_
