...
输入内容：
1. topic_name：研究主题名称。
2. active_findings：当前已有的多条 findings 列表，每条包含 finding_id、summary、detail、confidence、citations 等字段。
...
4. 每条合并后的 finding 必须保留所有来源的 citations，去重后合并。
5. merged_findings 最多输出 12 条，按 confidence 和时间新鲜度排序。
6. 每条 merged finding 的 summary 不超过 80 个中文字符。
