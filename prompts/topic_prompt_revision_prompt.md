...
3. 修订后的提示词必须仍然要求 LLM 对每条发现附带 citation（原始消息 ID 和原文片段），但**不要**定义 findings 的具体 JSON 字段结构。
4. 如果用户反馈模糊或矛盾，优先保留原提示词内容，并在 revision_note 中说明。
5. 版本号递增。
6. 修订后的提示词应可直接用于研究流程。
...
禁止事项：
1. 不要删除证据引用要求。
2. **严禁在 revised_prompt 中包含 JSON 输出 schema、示例 JSON、输出格式模板或字段定义。** 如果原提示词中存在这些内容，必须在修订时移除。
...
5. 所有字段必须存在，无内容字段使用空字符串或空数组，不要使用 null。
6. 必须输出合法 JSON 对象，不要输出 Markdown，不要包裹代码块。
